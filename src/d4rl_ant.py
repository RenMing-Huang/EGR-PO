...
class GoalReachingAnt(gym.Wrapper):
    ...
    def get_starting_boundary(self):
        ...
        return (0 - S / 2  - torso_x, 0 - S/2  - torso_y), (len(self._maze_map[0]) * S - torso_x - S/2 , len(self._maze_map) * S - torso_y - S/2 )
    ...
    def random_goals(self, sample_nums=4):
        self = self.env.env
        valid_cells = []
        goal_cells = []
        ...
        for _ in range(sample_nums):
            goals.append(valid_cells[np.random.choice(len(valid_cells))])
...
def plot_points(env, dataset, points, fig, ax, color_index=None, cmap='viridis'):
    # if color_list is None:
    #     from itertools import cycle
    #     color_cycle = plt.rcParams['axes.prop_cycle'].by_key()['color']
    #     color_list = cycle(color_cycle)
    mesh = ax.scatter(points[:, 0], points[:, 1], s=300, c=color_index, marker='o', alpha=0.5, cmap=plt.get_cmap(cmap))
    env.draw(ax)
    divider = make_axes_locatable(ax)
    cax = divider.append_axes('right', size='5%', pad=0.2)
    tick_locator = ticker.MaxNLocator(nbins=5)
    formatter = ticker.StrMethodFormatter("{x:.0f}")
    fig.colorbar(mesh, cax=cax, orientation='vertical', ticks=tick_locator, format=formatter)
...
def plot_trajectories(env, dataset, trajectories, fig, ax, color_list=None):
    ...
    for color, trajectory in zip(color_list, trajectories):
        ...
        all_x = obs[:, 0]
        all_y = obs[:, 1]
        ax.scatter(all_x, all_y, s=10, color=color, alpha=0.02)
        ax.scatter(all_x[-1], all_y[-1], s=100, color=color, marker='*', alpha=0.5)
...
def point_image(env, dataset, points, **kwargs):
    unwarp_env = env.env.env
    wight = len(unwarp_env._maze_map[0])
    height = len(unwarp_env._maze_map)
    ratio = wight / height


    fig = plt.figure(tight_layout=False, figsize=(6.4 * ratio, 6.4))
    canvas = FigureCanvas(fig)
    plot_points(env, dataset, points, fig, plt.gca(), **kwargs)
...
def trajectory_image(env, dataset, trajectories, **kwargs):
    ...
    fig = plt.figure(tight_layout=True, figsize=(6.4 * ratio, 6.4))
    canvas = FigureCanvas(fig)
...
def goals_image(env, dataset, starts, low_goals, choosed_idx, high_goal):
    fig = plt.figure(tight_layout=False)
    canvas = FigureCanvas(fig)
    ax = fig.add_subplot(111)
    env.draw(ax)
    
    for start, low_goal in zip(starts, low_goals):
        # for goal in low_goal:
        ax.scatter(low_goal[0], low_goal[1], s=100, c='green', marker='*', alpha=0.8)
        # plot start
        ax.scatter(start[0], start[1], s=100, c='black', marker='o', alpha=1)
        # plot choosed goal
        # ax.scatter(low_goals[choosed_idx][0], low_goals[choosed_idx][1], s=150, c='red', marker='*', alpha=0.8)
        # plot high goal
    ax.scatter(high_goal[0], high_goal[1], s=200, c='orange', marker='*', alpha=1)
    image = get_canvas_image(canvas)
    plt.close(fig)
    return image
...
def gcvalue_image(env, dataset, value_fn, target):
    """
    ...
    points = [point_target, point2, point3, point4]
