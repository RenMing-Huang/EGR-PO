...
def optimize(loss: torch.Tensor, agent: "JointTrainAgent", optim_keys):
    active = [agent.network.optims[k] for k in optim_keys if k in agent.network.optims]
    for opt in active:
        opt.zero_grad()
    loss.backward()
    for opt in active:
...
def compute_actor_loss(agent: "JointTrainAgent", batch):
    ...
    with torch.no_grad():
        v1, v2 = agent.network.value(batch['achieved_goals'], cur_goals)
        nv1, nv2 = agent.network.value(batch['next_achieved_goals'], cur_goals)

    v = (v1 + v2) / 2
    nv = (nv1 + nv2) / 2
    adv = nv - v
    exp_a = torch.clamp(torch.exp(adv * agent.config['temperature']), max=100.0)
    ...
    optimize(actor_loss, agent, ['actor_optim', 'policy_state_optim'])

    actor_loss_val = actor_loss.detach().cpu().numpy()
    adv_np = adv.detach().cpu().numpy()
    return actor_loss_val, {
        'actor_loss': actor_loss_val,
        'adv': adv_np.mean(),
        'bc_log_probs': log_probs.mean(),
        'adv_median': np.median(adv_np),
        'mse': torch.mean((dist.mean - batch['actions']) ** 2).detach().cpu().numpy(),
        'std': dist.stddev.mean().detach().cpu().numpy(),
    }
...
def compute_high_actor_loss(agent: "JointTrainAgent", batch, pred_goal=None):
    ...
    with torch.no_grad():
        ...
        nv1, nv2 = agent.network.value(batch['high_targets'], cur_goals)

    v = (v1 + v2) / 2
    nv = (nv1 + nv2) / 2
    adv = nv - v
    exp_a = torch.clamp(torch.exp(adv * agent.config['high_temperature']), max=100.0).unsqueeze(-1)
    ...
    cond = torch.cat([batch['observations'], cur_goals], dim=-1)
    bc_loss = high_policy.actor.loss(target, cond)
    generated_target = high_policy.actor(cond)
...
def compute_value_loss(agent: "JointTrainAgent", batch):
    ...
    value_loss = (
        expectile_loss(adv, q1 - v1, agent.config['pretrain_expectile']).mean()
        + expectile_loss(adv, q2 - v2, agent.config['pretrain_expectile']).mean()
    )

    optimize(value_loss, agent, ['value_optim', 'value_state_optim', 'value_goal_optim'])

    value_loss_val = value_loss.detach().cpu().numpy()
    adv_np = adv.detach().cpu().numpy()
    return value_loss_val, {
        ...
        'v min': v1.min(),
        'v mean': v1.mean(),
        'abs adv mean': np.abs(adv_np).mean(),
        'adv mean': adv_np.mean(),
        'adv max': adv_np.max(),
        'adv min': adv_np.min(),
        'accept prob': (adv_np >= 0).mean(),
    }
...
class JointTrainAgent(iql.IQLAgent):
    ...
    def pretrain_update(agent, pretrain_batch, seed=None, value_update=True, actor_update=True, high_actor_update=True, pred_goal=None):
        batch = {k: torch.tensor(v).float().to(device) for k, v in pretrain_batch.items()}

        info = {}

        if value_update:
            value_loss, value_info = compute_value_loss(agent, batch)
        ...
        if actor_update:
            actor_loss, actor_info = compute_actor_loss(agent, batch)
        ...
        if high_actor_update and agent.config['use_waypoints']:
            high_actor_loss, high_actor_info = compute_high_actor_loss(agent, batch, pred_goal)
    ...
    def sample_actions(agent,
        ...
        actions = dist.mean if num_samples is None else dist.sample((num_samples,))
        if not discrete:
    ...
    def sample_high_actions(agent,
        ...
        if num_samples:
            observations = observations.repeat_interleave(num_samples, dim=0)
            goals = goals.repeat_interleave(num_samples, dim=0)
    ...
    def get_policy_rep(agent,
                       *,
                       ...
                       bases: torch.Tensor = None) -> torch.Tensor:
...
def create_learner(
    ...
    if visual:
        ...
        def make_encoder(bottleneck, in_chan=3):
            if bottleneck:
                return RelativeRepresentation(input_dim=obs_dim, in_channel=in_chan, rep_dim=rep_dim,
                                              hidden_dims=(*value_hidden_dims[:-1], rep_dim),
                                              visual=True, module=visual_encoder, layer_norm=use_layer_norm,
                                              rep_type=rep_type, bottleneck=True)
            return RelativeRepresentation(input_dim=obs_dim, in_channel=in_chan, rep_dim=value_hidden_dims[-1],
                                          hidden_dims=(*value_hidden_dims,), visual=True, module=visual_encoder,
                                          layer_norm=use_layer_norm, rep_type=rep_type, bottleneck=False)
        ...
        def make_encoder(bottleneck):
            if bottleneck:
                return RelativeRepresentation(input_dim=obs_dim, rep_dim=rep_dim,
                                              hidden_dims=(*value_hidden_dims[:-1], rep_dim),
                                              layer_norm=use_layer_norm, rep_type=rep_type, bottleneck=True)
            return RelativeRepresentation(input_dim=obs_dim, rep_dim=value_hidden_dims[-1],
    ...
    if visual:
        value_input_dim = (value_hidden_dims[-1] if value_state_encoder else obs_dim) + \
                          (rep_dim if value_goal_encoder else obs_dim)
    else:
        value_input_dim = (rep_dim if value_state_encoder else obs_dim) + \
                          (rep_dim if value_goal_encoder else goal_dim)

    value_def = MonolithicVF(input_dim=value_input_dim, hidden_dims=value_hidden_dims,
                             use_layer_norm=use_layer_norm, rep_dim=rep_dim)
    target_value_def = copy.deepcopy(value_def)
    for p in target_value_def.parameters():
        p.requires_grad = False

    if visual:
        actor_input_dim = (value_hidden_dims[-1] if policy_state_encoder else obs_dim) + \
                          (rep_dim if use_waypoints else value_hidden_dims[-1])
    else:
        actor_input_dim = (rep_dim if policy_state_encoder else obs_dim) + \
                          (rep_dim if (policy_goal_encoder or (use_waypoints and value_goal_encoder)) else obs_dim)

    if discrete:
        action_dim = actions[0] + 1
        actor_def = DiscretePolicy(actor_input_dim, actor_hidden_dims, action_dim=action_dim)
    else:
        action_dim = actions.shape[-1]
        actor_def = Policy(actor_input_dim, actor_hidden_dims, action_dim=action_dim,
                           log_std_min=-5.0, state_dependent_std=False, tanh_squash_distribution=False)

    if visual:
        high_input_dim = (value_hidden_dims[-1] if high_policy_state_encoder else obs_dim) + \
                         (value_hidden_dims[-1] if (high_policy_goal_encoder or use_waypoints) else obs_dim)
    else:
        high_input_dim = (rep_dim if high_policy_state_encoder else obs_dim) + \
                         (rep_dim if high_policy_goal_encoder else goal_dim)

    high_action_dim = obs_dim if not use_rep else rep_dim
    high_actor_def = Diffusion_QL(
        state_dim=high_input_dim,
    ...
    return JointTrainAgent(rng, network=network_def, critic=None, value=None, target_value=None, actor=None, config=config)
