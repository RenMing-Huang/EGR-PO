...
def build_net(layer_shape, hidden_activation, output_activation, use_norm=True):
	'''Build net with for loop'''
	layers = []
	for j in range(len(layer_shape)-1):
		act = hidden_activation if j < len(layer_shape)-2 else output_activation
		if use_norm and j != len(layer_shape)-2:
			layers += [nn.Linear(layer_shape[j], layer_shape[j+1]), act(), nn.LayerNorm(layer_shape[j+1])]
		else:
...
class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, hid_shape, hidden_activation=nn.ReLU, output_activation=nn.ReLU):
		...
		self.a_net = build_net(layers, hidden_activation, output_activation, use_norm=True)
...
class Q_Critic(nn.Module):
	def __init__(self, state_dim, action_dim, hid_shape):
		...
		self.Q = build_net(layers, nn.ReLU, nn.Identity, use_norm=True)
...
class Double_Q_Critic(nn.Module):
	def __init__(self, state_dim, action_dim, hid_shape):
		...
		self.Q_1 = build_net(layers, nn.ReLU, nn.Identity, use_norm=True)
		self.Q_2 = build_net(layers, nn.ReLU, nn.Identity, use_norm=True)
