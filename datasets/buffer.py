...
class ReplayBuffer:
    ...
    def __init__(self, max_n_episodes, max_path_length, termination_penalty, discounts):
        self._dict = {
            'path_lengths': np.zeros(max_n_episodes, dtype=np.int32),
            'sum_rewards': np.zeros((max_n_episodes, max_path_length), dtype=np.float32),
            'discounted_rewards': np.zeros(max_n_episodes, dtype=np.float32),
        }
        self._count = 0
        ...
        self.max_discounted_return = -1000
    ...
    def items(self):
        return {k: v for k, v in self._dict.items()
                if k != 'path_lengths' and k != 'discounted_rewards'}.items()
    ...
    def add_path(self, path):
        ...
        discounts = self.discounts[:path_length]
        discounted_rewards = np.sum(discounts * self._dict['rewards'][self._count, :path_length])
        self._dict['discounted_rewards'][self._count] = discounted_rewards
        self.max_discounted_return = max(self.max_discounted_return, discounted_rewards)
        self._dict['sum_rewards'][self._count, :path_length] = np.cumsum(self._dict['rewards'][self._count, :path_length])
        ## record path length
    ...
    def finalize(self):
        ## remove extra slots
        for key in self.keys + ['path_lengths', 'sum_rewards', 'discounted_rewards']:
            self._dict[key] = self._dict[key][:self._count]
        self._add_attributes()
        print(f'[ datasets/buffer ] Finalized replay buffer | {self._count} episodes')
        print(f'[ datasets/buffer ] Max discounted return: {self.max_discounted_return:.2f}')
